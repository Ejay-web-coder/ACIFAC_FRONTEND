...
import { createSavingsRequest, fetchMembers, fetchSavingsRecords } from '../services/membersApi';
...
// Savings deposits have their own ledger (savings_transactions); share capital is separate.
...
export function Savings({ userRole, view }: { userRole: UserRole; view: SavingsView }) {
  ...
  useLiveRefresh(['share_contributions', 'members'], () => { void loadSavings(); });

  // Total of every contribution in the database (not only the rows loaded on this page).
  ...
  return <>
    <Overview records={records} members={members} total={total} totals={serverTotals} filtered={filtered} search={search} setSearch={setSearch} memberFilter={memberFilter} setMemberFilter={setMemberFilter} typeFilter={typeFilter} setTypeFilter={setTypeFilter} dateFilter={dateFilter} setDateFilter={setDateFilter} onAddSavings={() => setShowAddModal(true)} />
...
function Summary({ title, value, icon: Icon, color }: { title: string; value: string; icon: typeof Wallet; color: string }) { return <StatCard label={title} value={value} icon={Icon} tone={color.includes('green') ? 'dark' : 'green'} />; }
function Overview({ records, members, total, totals, filtered, search, setSearch, memberFilter, setMemberFilter, typeFilter, setTypeFilter, dateFilter, setDateFilter, onAddSavings }: { records: SavingsRecord[]; members: Array<{ id: number; memberId: string; memberName: string }>; total: number; totals: { today: number; month: number; members: number } | null; filtered: SavingsRecord[]; search: string; setSearch: (value: string) => void; memberFilter: string; setMemberFilter: (value: string) => void; typeFilter: string; setTypeFilter: (value: string) => void; dateFilter: string; setDateFilter: (value: string) => void; onAddSavings: () => void }) { const [showHistory, setShowHistory] = useState(false); const today = totals?.today ?? 0; const month = totals?.month ?? 0; const membersWithSavings = totals?.members ?? 0; return <div className="space-y-6"><Header description="Member savings deposits only. Share capital is tracked separately in member records." action={<button type="button" onClick={onAddSavings} className="inline-flex items-center justify-center gap-2 rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700"><Plus className="h-4 w-4" />Add Savings</button>} /><div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4"><Summary title="Total Savings" value={peso(total)} icon={Wallet} color="bg-green-50 text-green-700" /><Summary title="Members with Savings" value={String(membersWithSavings)} icon={Wallet} color="bg-blue-50 text-blue-700" /><Summary title="Today's Savings" value={peso(today)} icon={Plus} color="bg-orange-50 text-orange-700" /><Summary title="This Month" value={peso(month)} icon={Wallet} color="bg-purple-50 text-purple-700" /></div><div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between gap-3"><h3 className="font-bold text-gray-900">Recent Savings Activity</h3><button type="button" onClick={() => setShowHistory(current => !current)} className="text-sm font-medium text-green-700 hover:text-green-800">{showHistory ? 'Hide history' : 'View history'}</button></div>{showHistory ? <History records={filtered} members={members} search={search} setSearch={setSearch} memberFilter={memberFilter} setMemberFilter={setMemberFilter} typeFilter={typeFilter} setTypeFilter={setTypeFilter} dateFilter={dateFilter} setDateFilter={setDateFilter} /> : records.length ? <div className="space-y-3">{records.slice(0, 5).map((record) => <div key={record.id} className="flex items-center justify-between gap-3 border-b border-gray-100 pb-3 last:border-0 last:pb-0"><div className="min-w-0"><p className="truncate text-sm font-medium text-gray-900">{record.memberName}</p><p className="text-xs text-gray-500">{record.type} · {formatDate(record.date)}</p></div><span className="shrink-0 text-sm font-semibold text-green-700">+{peso(record.amount)}</span></div>)}</div> : <p className="py-8 text-center text-sm text-gray-500">No savings records yet.</p>}</div></div>; }
...
function AddSavings({ form, errors, update, submit, onCancel, members }: { form: { member: string; memberId: string; amount: string; date: string; type: TransactionType; reference: string; notes: string }; errors: Record<string, string>; update: (field: string, value: string) => void; submit: (event: FormEvent) => void; onCancel?: () => void; members: Array<{ id: number; memberId: string; memberName: string }> }) { return <form onSubmit={submit} className="space-y-4 p-6"><div className="grid grid-cols-1 gap-4 md:grid-cols-2"><Field label="Member" error={errors.member}><select value={form.member} onChange={(event) => { const selected = members.find((member) => member.memberName === event.target.value); update('member', event.target.value); update('memberId', selected?.memberId || ''); }} className="w-full rounded-xl border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-600"><option value="">Select member</option>{members.map((member) => <option key={member.memberId}>{member.memberName}</option>)}</select></Field><Field label="Member ID"><input readOnly value={form.memberId} className="w-full rounded-xl border border-gray-300 bg-gray-50 px-3 py-2" placeholder="Auto-filled" /></Field><Field label="Savings Amount" error={errors.amount}><input type="number" min="0.01" step="0.01" value={form.amount} onChange={(event) => update('amount', event.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-600" placeholder="0.00" /></Field><Field label="Date" error={errors.date}><input type="date" value={form.date} onChange={(event) => update('date', event.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-600" /></Field><Field label="Transaction Type"><select value={form.type} onChange={(event) => update('type', event.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-600">{types.map((type) => <option key={type}>{type}</option>)}</select></Field><Field label="Reference"><input value={form.reference} onChange={(event) => update('reference', event.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-600" placeholder="Optional reference" /></Field><div className="md:col-span-2"><Field label="Notes"><textarea value={form.notes} onChange={(event) => update('notes', event.target.value)} className="min-h-24 w-full rounded-xl border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-600" placeholder="Optional notes" /></Field></div></div><div className="flex gap-3 border-t border-gray-200 pt-4"><button type="submit" className="flex-1 rounded-xl bg-green-600 px-4 py-2 font-medium text-white hover:bg-green-700">Add Savings</button><button type="button" onClick={onCancel} className="flex-1 rounded-lg bg-gray-200 px-4 py-2 font-medium text-gray-700 hover:bg-gray-300">Cancel</button></div></form>; }
function History({ records, members, search, setSearch, memberFilter, setMemberFilter, typeFilter, setTypeFilter, dateFilter, setDateFilter }: { records: SavingsRecord[]; members: Array<{ id: number; memberId: string; memberName: string }>; search: string; setSearch: (value: string) => void; memberFilter: string; setMemberFilter: (value: string) => void; typeFilter: string; setTypeFilter: (value: string) => void; dateFilter: string; setDateFilter: (value: string) => void }) { const pages = usePagination(records, { resetKey: `${search}|${memberFilter}|${typeFilter}|${dateFilter}` }); return <div className="space-y-6"><Header title="Savings History" description="Review member deposit transactions separately from share capital contributions" /><div className="rounded-2xl border border-gray-200 bg-white shadow-[var(--shadow-card)]"><div className="flex flex-col gap-3 border-b border-gray-200 p-4 xl:flex-row"><div className="relative min-w-0 flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search member, ID, or reference" className="input-style pl-9" /></div><div className="flex flex-col gap-2 sm:flex-row"><div className="flex items-center gap-2"><Filter className="h-4 w-4 shrink-0 text-gray-500" /><select value={memberFilter} onChange={(event) => setMemberFilter(event.target.value)} className="input-style"><option value="all">All Members</option>{members.map((member) => <option key={member.memberId} value={member.memberId}>{member.memberName}</option>)}</select></div><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} className="input-style"><option value="all">All Types</option>{types.map((type) => <option key={type}>{type}</option>)}</select><input type="date" aria-label="Filter by date" value={dateFilter} onChange={(event) => setDateFilter(event.target.value)} className="input-style" /></div></div><div className="overflow-x-auto"><table className="w-full min-w-[1050px]"><thead className="bg-gray-50"><tr>{['Date', 'Member ID', 'Member Name', 'Transaction Type', 'Amount', 'Reference / Notes', 'Status', 'Actions'].map((heading) => <th key={heading} className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500">{heading}</th>)}</tr></thead><tbody className="divide-y divide-gray-200">{records.length ? pages.pageItems.map((record) => <tr key={record.id} className="hover:bg-gray-50"><td className="px-4 py-3 text-sm text-gray-600">{formatDate(record.date)}</td><td className="px-4 py-3 text-sm text-gray-600">{record.memberId}</td><td className="px-4 py-3 text-sm font-medium text-gray-900">{record.memberName}</td><td className="px-4 py-3 text-sm text-gray-600">{record.type}</td><td className="px-4 py-3 text-sm font-semibold text-gray-900">{peso(record.amount)}</td><td className="px-4 py-3 text-sm text-gray-600">{record.reference}{record.notes && <span className="block text-xs text-gray-400">{record.notes}</span>}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-medium ${record.status === 'Completed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{record.status}</span></td><td className="px-4 py-3"><button type="button" onClick={() => toast.info(`Savings ${record.reference}`, { description: `${record.memberName} · ${peso(record.amount)}` })} className="inline-flex items-center gap-1 text-sm font-medium text-blue-600 hover:text-blue-800"><Eye className="h-4 w-4" />View</button></td></tr>) : <tr><td colSpan={8} className="px-4 py-12 text-center text-sm text-gray-500">No savings records match your filters.</td></tr>}</tbody></table></div><Pagination page={pages.page} totalPages={pages.totalPages} total={pages.total} pageSize={pages.pageSize} onPageChange={pages.setPage} onPageSizeChange={pages.setPageSize} label="deposits" /></div></div>; }
